...
from src.common.helper.utils import map_columns, extract_numeric
...
def transform_products(data: pd.DataFrame, table_name: str) -> pd.DataFrame | None:
    """
    ...
    try:
        ...
        data = data.rename(columns={'product_id': 'nk_product_id', 'store_branch': 'sk_store_branch'})

        dim_store_branch = extract_table(DWH_DB_URI, "public", "dim_store_branch")
        data = map_columns(data, dim_store_branch, 'sk_store_branch', 'store_name', 'sk_store_id')
        data[['unit_price', 'cost_price']] = data[['unit_price', 'cost_price']].applymap(extract_numeric)
